...
bool HexEditor::addressArea() const {
    return _addressArea;
}
...
qint64 HexEditor::addressOffset() const {
...
bool HexEditor::asciiArea() const {
    return _asciiArea;
}
...
bool HexEditor::highlighting() const {
...
bool HexEditor::overwriteMode() const {
...
bool HexEditor::dynamicBytesPerLine() const {
...
void HexEditor::paintEvent(QPaintEvent *event) {
    ...
    if (event->rect() != _cursorRect) {
        ...
        if (_addressArea) {
            QString address;
            for (int row = 0, pxPosY = _pxCharHeight;
                 row <= (_dataShown.size() / _bytesPerLine); row++, pxPosY += _pxCharHeight) {
                address = QString("%1").arg(_bPosFirst + row * _bytesPerLine + _addressOffset, _addrDigits, 16,
                                            QChar('0'));
    ...
    if ((hexPositionInShowData >= 0) && (hexPositionInShowData < _hexDataShown.size())) {
        ...
        if (_readOnly) {
            ...
            painter.fillRect(QRect(_pxCursorX - pxOfsX, _pxCursorY - _pxCharHeight + _pxSelectionSub, _pxCharWidth,
                                   _pxCharHeight), color);
        } else {
            if (_blink && hasFocus())
        ...
        if (_editAreaIsAscii) {
            ...
            painter.drawText(_pxCursorX - pxOfsX, _pxCursorY,
                             hexCaps() ? _hexDataShown.mid(hexPositionInShowData, 1).toUpper() : _hexDataShown.mid(
