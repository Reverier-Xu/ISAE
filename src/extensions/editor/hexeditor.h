...
/** \mainpage
HexEditor is a binary editor widget for Qt.
...
/** HexEditor is a hex editor widget written in C++ for the Qt (Qt4, Qt5) framework.
...
HexEditor takes the data of a QByteArray (setData()) and shows it. You can use
...
Normally HexEditor works in the overwrite mode. You can set overwrite mode(false)
...
HexEditor comes with undo/redo functionality. All changes can be undone, by
...
HexEditor is based on QIODevice, that's why HexEditor can handle big amounts of
data. The size of edited data can be more then two gigabytes without any
restrictions.
*/
class QHEXEDIT_API HexEditor : public QAbstractScrollArea {
Q_OBJECT
    ...
    */
    ...
    in HexEditor. Every byte in data has two cursor positions: the lower and upper
    ...
    */
    ...
    /*! Property data holds the content of HexEditor. Call setData() to set the
    content of HexEditor, data() returns the actual content. When calling setData()
    with a QByteArray as argument, HexEditor creates a internal copy of the data
...
public:
    ...
    \param parent Parent widget of HexEditor.
    */
    explicit HexEditor(QWidget *parent = nullptr);

    // Access to data of qhexedit

    /*! Sets the data of HexEditor. The QIODevice will be opened just before reading
    ...
    */
    ...
    /*! Set Font of HexEditor
     * \param font
     */
    void setFont(const QFont &font);

    /*! Gives back a formatted image of the content of HexEditor
    */
...
public:
    ...
    // Properties
    [[nodiscard]] bool addressArea() const;
    ...
    [[nodiscard]] qint64 addressOffset() const;
    ...
    [[nodiscard]] bool asciiArea() const;

    void setAsciiArea(bool asciiArea);

    [[nodiscard]] int bytesPerLine() const;

    void setBytesPerLine(int count);

    [[nodiscard]] qint64 cursorPosition() const;
    ...
    [[nodiscard]] bool hexCaps() const;

    void setDynamicBytesPerLine(bool isDynamic);

    [[nodiscard]] bool dynamicBytesPerLine() const;

    [[nodiscard]] bool highlighting() const;
    ...
    [[nodiscard]] bool overwriteMode() const;
...
private:
    ...
    bool _addressArea;                          // left area of HexEditor
    ...
    QByteArray _data;                           // HexEditor's data, when setup with QByteArray
