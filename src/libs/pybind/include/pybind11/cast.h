            ...
            PYBIND11_NOINLINE bool load_impl(handle src, bool convert) {
                ...
                if (srctype == typeinfo->type) {
                    ...
                    if (bases.size() == 1 && (no_cpp_mi || bases.front()->type == typeinfo->type)) {
                        ...
                        for (auto base : bases) {
                            if (no_cpp_mi ? PyType_IsSubtype(base->type, typeinfo->type) : base->type ==
                                                                                           typeinfo->type) {
...
// so, copy constructability depends on whether the value_type is copy constructible.
        ...
        struct is_copy_constructible<Container, enable_if_t<all_of<
...
// The same problems arise with std::is_copy_assignable, so we use the same workaround.
        ...
        template<typename Container>
        struct is_copy_assignable<Container, enable_if_t<all_of<
    ...
    public: \
        ...
        public:
            ...
            static typename std::enable_if<!std::is_floating_point<U>::value && std::is_signed<U>::value &&
                                           (sizeof(U) <= sizeof(long)), handle>::type
            cast(U src, return_value_policy /* policy */, handle /* parent */) {
            ...
            static typename std::enable_if<!std::is_floating_point<U>::value && std::is_unsigned<U>::value &&
                                           (sizeof(U) <= sizeof(unsigned long)), handle>::type
            cast(U src, return_value_policy /* policy */, handle /* parent */) {
            ...
            static handle cast_impl(T &&src, return_value_policy policy, handle parent, index_sequence<Is...>) {
                std::array<object, size> entries {{
                                                          reinterpret_steal<object>(make_caster<Ts>::cast(
                                                                  std::get<Is>(std::forward<T>(src)), policy,
                                                                  parent))...
                                                  }};
...
// C++ type -> py::object
    ...
    template<typename T>
    T handle::cast() const { return pybind11::cast<T>(*this); }

    template<>
    inline void handle::cast() const { return; }
...
// - Otherwise (not movable), copy.
    ...
    template<typename T>
    T object::cast() const &{ return pybind11::cast<T>(*this); }

    template<typename T>
    T object::cast() &&{ return pybind11::cast<T>(std::move(*this)); }

    template<>
    inline void object::cast() const &{ return; }

    template<>
    inline void object::cast() &&{ return; }
