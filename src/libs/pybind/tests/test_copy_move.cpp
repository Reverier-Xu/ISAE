...
struct type_caster<MoveOnlyInt> {
    PYBIND11_TYPE_CASTER(MoveOnlyInt, _(
    "MoveOnlyInt"));
...
struct type_caster<MoveOrCopyInt> {
    PYBIND11_TYPE_CASTER(MoveOrCopyInt, _(
    "MoveOrCopyInt"));
...
TEST_SUBMODULE(copy_move_policies, m) {
    ...
    m.def("move_copy_nested",
          [](std::pair<MoveOnlyInt, std::pair<std::tuple<MoveOrCopyInt, CopyOnlyInt, std::tuple<MoveOnlyInt>>, MoveOrCopyInt>> x) {
