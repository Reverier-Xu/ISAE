        ...
        try:
            if callable(cmd):
                ...
                while self.UNIQUE_PATH_DELIMITER in cmd:
                    ...
                    cmd = cmd.replace(self.UNIQUE_PATH_DELIMITER + need_unique_path + self.UNIQUE_PATH_DELIMITER,
                                      unique_path)
            ...
            binwalk.core.common.warning(
                "Extractor.execute failed to run external extractor '%s': %s, '%s' might not be installed correctly" % (
                str(cmd), str(e), str(cmd)))
            retval = None
