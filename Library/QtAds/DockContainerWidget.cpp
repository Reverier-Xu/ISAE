...
inline char toHexLower(uint value) { return "0123456789abcdef"[value & 0xF]; }
...
QByteArray qByteArrayToHex(const QByteArray& src, char separator) {
    ...
    for (int i = 0, o = 0; i < src.size(); ++i) {
        ...
        if ((separator) && (o < length)) hexData[o++] = separator;
    }
    return hex;
}
#endif
...
class DockContainerWidgetPrivate {
   public:
     ...
     */
    CDockAreaWidget* dockWidgetIntoContainer(DockWidgetArea area,
                                             CDockWidget* Dockwidget);

    /**
     * Adds dock widget to a existing DockWidgetArea
     */
    CDockAreaWidget* dockWidgetIntoDockArea(DockWidgetArea area,
                                            CDockWidget* Dockwidget,
    ...
    CDockSplitter* newSplitter(Qt::Orientation orientation,
        ...
        s->setOpaqueResize(CDockManager::configFlags().testFlag(
...
void DockContainerWidgetPrivate::onVisibleDockAreaCountChanged() {
    ...
    if (TopLevelDockArea) {
        ...
        this->TopLevelDockArea->titleBarButton(TitleBarButtonUndock)
            ->setVisible(true);
        this->TopLevelDockArea->titleBarButton(TitleBarButtonClose)
            ->setVisible(true);
...
void DockContainerWidgetPrivate::dropIntoContainer(
    ...
    CDockContainerWidget* FloatingDockContainer =
...
void DockContainerWidgetPrivate::dropIntoSection(
    ...
    auto InsertParam = internal::dockAreaInsertParameters(area);
    auto NewDockAreas =
        FloatingWidget->dockContainer()->findChildren<CDockAreaWidget*>(
            QString(), Qt::FindChildrenRecursively);
    QSplitter* TargetAreaSplitter =
        internal::findParent<QSplitter*>(TargetArea);
    ...
    auto Widget = FloatingWidget->dockContainer()->findChild<QWidget*>(
    ...
    if (TargetAreaSplitter->orientation() == InsertParam.orientation()) {
        ...
        if ((FloatingSplitter->orientation() != InsertParam.orientation()) &&
            ...
            TargetAreaSplitter->insertWidget(
                AreaIndex + InsertParam.insertOffset(), Widget);
        } else {
...
void DockContainerWidgetPrivate::moveIntoCenterOfSection(
    QWidget* Widget, CDockAreaWidget* TargetArea) {
    ...
    if (DroppedDockWidget) {
        ...
        if (Splitter == RootSplitter &&
            ...
            if (InsertParam.append() &&
                Splitter->lastWidget() == DroppedDockArea) {
                return;
...
bool DockContainerWidgetPrivate::restoreSplitter(CDockingStateReader& s,
    ...
    // In version 0 we had a small bug. The "|" indicated a vertical
    // orientation, but this is wrong, because only the splitter handle is
    // vertical, the layout of the splitter is a horizontal layout. We fix this
    // here
    if (s.fileVersion() == 0) {
...
//============================================================================
CDockAreaWidget* DockContainerWidgetPrivate::dockWidgetIntoContainer(
...
//============================================================================
CDockAreaWidget* DockContainerWidgetPrivate::dockWidgetIntoDockArea(
    DockWidgetArea area, CDockWidget* Dockwidget,
    CDockAreaWidget* TargetDockArea) {
    if (CenterDockWidgetArea == area) {
    ...
    QSplitter* TargetAreaSplitter =
        internal::findParent<QSplitter*>(TargetDockArea);
...
void CDockContainerWidget::removeDockArea(CDockAreaWidget* area) {
    ...
    if (auto p =
            std::find(cache, cache + sizeof(cache) / sizeof(cache[0]), area)) {
...
//============================================================================
CDockAreaWidget* CDockContainerWidget::dockAreaAt(
...
void CDockContainerWidget::dropFloatingWidget(
    ...
    if (Dropped) {
        ...
        // If we dropped a floating widget with only one single dock widget,
        // then we drop a top level widget that changes from floating to docked
        // now
