...
struct DockAreaTitleBarPrivate {
     ...
     */
    IFloatingWidget* makeAreaFloating(const QPoint& Offset,
...
void DockAreaTitleBarPrivate::createButtons() {
    ...
    TabsMenuButton = new CTitleBarButton(
        testConfigFlag(CDockManager::DockAreaHasTabsMenuButton));
    ...
    // Undock button
    UndockButton = new CTitleBarButton(
        testConfigFlag(CDockManager::DockAreaHasUndockButton));
    ...
    // Close button
    CloseButton = new CTitleBarButton(
        testConfigFlag(CDockManager::DockAreaHasCloseButton));
...
//============================================================================
IFloatingWidget* DockAreaTitleBarPrivate::makeAreaFloating(
    const QPoint& Offset, eDragState DragState) {
    QSize Size = DockArea->size();
    this->DragState = DragState;
    bool OpaqueUndocking =
        CDockManager::configFlags().testFlag(CDockManager::OpaqueUndocking) ||
...
//============================================================================
CDockAreaTabBar* CDockAreaTitleBar::tabBar() const { return d->TabBar; }
...
void CDockAreaTitleBar::mouseMoveEvent(QMouseEvent* ev) {
    ...
    // If this is the last dock area in a floating dock container it does not
    // make sense to move it to a new floating widget and leave this one empty
...
void CDockAreaTitleBar::contextMenuEvent(QContextMenuEvent* ev) {
    ...
    Menu.addSeparator();
    Action =
        Menu.addAction(tr("Close Area"), this, SLOT(onCloseButtonClicked()));
    Action->setEnabled(
...
//============================================================================
CTitleBarButton::CTitleBarButton(bool visible /*= true*/,
                                 QWidget* parent /*= nullptr*/)
    : tTitleBarButton(parent),
      Visible(visible),
      HideWhenDisabled(DockAreaTitleBarPrivate::testConfigFlag(
...
bool CTitleBarButton::event(QEvent* ev) {
    if (QEvent::EnabledChange == ev->type() && HideWhenDisabled) {
        ...
        // Calling setVisible() directly here doesn't work well when button is
        // expected to be shown first time
        QMetaObject::invokeMethod(this, "setVisible", Qt::QueuedConnection,
                                  Q_ARG(bool, isEnabled()));
    }
