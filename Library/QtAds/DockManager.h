   ...
   public:
    ...
    enum eConfigFlag {
        ActiveTabHasCloseButton =
            0x0001,  //!< If this flag is set, the active tab in a tab area has
                     //!< a close button
        DockAreaHasCloseButton =
            0x0002,  //!< If the flag is set each dock area has a close button
        DockAreaCloseButtonClosesTab =
            0x0004,  //!< If the flag is set, the dock area close button closes
                     //!< the active tab, if not set, it closes the complete
                     //!< dock area
        OpaqueSplitterResize =
            0x0008,  //!< See QSplitter::setOpaqueResize() documentation
        XmlAutoFormattingEnabled =
            0x0010,  //!< If enabled, the XML writer automatically adds
                     //!< line-breaks and indentation to empty sections between
                     //!< elements (ignorable whitespace).
        XmlCompressionEnabled =
            0x0020,  //!< If enabled, the XML output will be compressed and is
                     //!< not human readable anymore
        TabCloseButtonIsToolButton =
            0x0040,  //! If enabled the tab close buttons will be QToolButtons
                     //! instead of QPushButtons - disabled by default
        AllTabsHaveCloseButton =
            0x0080,  //!< if this flag is set, then all tabs that are closable
                     //!< show a close button
        RetainTabSizeWhenCloseButtonHidden =
            0x0100,  //!< if this flag is set, the space for the close button is
                     //!< reserved even if the close button is not visible
        OpaqueUndocking =
            0x0200,  ///< If enabled, the widgets are immediately undocked into
                     ///< floating widgets, if disabled, only a draw preview is
                     ///< undocked and the real undocking is deferred until the
                     ///< mouse is released
        DragPreviewIsDynamic =
            0x0400,  ///< If opaque undocking is disabled, this flag defines the
                     ///< behavior of the drag preview window, if this flag is
                     ///< enabled, the preview will be adjusted dynamically to
                     ///< the drop area
        DragPreviewShowsContentPixmap =
            0x0800,  ///< If opaque undocking is disabled, the created drag
                     ///< preview window shows a copy of the content of the dock
                     ///< widget / dock are that is dragged
        DragPreviewHasWindowFrame =
            0x1000,  ///< If opaque undocking is disabled, then this flag
                     ///< configures if the drag preview is frameless or looks
                     ///< like a real window
        AlwaysShowTabs =
            0x2000,  ///< If this option is enabled, the tab of a dock widget is
                     ///< always displayed - even if it is the only visible dock
                     ///< widget in a floating widget.
        DockAreaHasUndockButton =
            0x4000,  //!< If the flag is set each dock area has an undock button
        DockAreaHasTabsMenuButton = 0x8000,  //!< If the flag is set each dock
                                             //!< area has a tabs menu button
        DockAreaHideDisabledButtons =
            0x10000,  //!< If the flag is set disabled dock area buttons will
                      //!< not appear on the toolbar at all (enabling them will
                      //!< bring them back)
        DockAreaDynamicTabsMenuButtonVisibility =
            0x20000,  //!< If the flag is set, the tabs menu button will be
                      //!< shown only when it is required - that means, if the
                      //!< tabs are elided. If the tabs are not elided, it is
                      //!< hidden
        FloatingContainerHasWidgetTitle =
            0x40000,  //!< If set, the Floating Widget window title reflects the
                      //!< title of the current dock widget otherwise it
                      //!< displays application name as window title
        FloatingContainerHasWidgetIcon =
            0x80000,  //!< If set, the Floating Widget icon reflects the icon of
                      //!< the current dock widget otherwise it displays
                      //!< application icon
        HideSingleCentralWidgetTitleBar =
            0x100000,  //!< If there is only one single visible dock widget in
                       //!< the main dock container (the dock manager) and if
                       //!< this flag is set, then the titlebar of this dock
                       //!< widget will be hidden this only makes sense for non
                       //!< draggable and non floatable widgets and enables the
                       //!< creation of some kind of "central" widget

        DefaultDockAreaButtons =
            DockAreaHasCloseButton | DockAreaHasUndockButton |
            DockAreaHasTabsMenuButton,  ///< default configuration of dock area
                                        ///< title bar buttons

        DefaultBaseConfig =
            DefaultDockAreaButtons | ActiveTabHasCloseButton |
            XmlCompressionEnabled |
            FloatingContainerHasWidgetTitle,  ///< default base configuration
                                              ///< settings

        DefaultOpaqueConfig =
            DefaultBaseConfig | OpaqueSplitterResize |
            OpaqueUndocking,  ///< the default configuration with opaque
                              ///< operations - this may cause issues if ActiveX
                              ///< or Qt 3D windows are involved

        DefaultNonOpaqueConfig =
            DefaultBaseConfig |
            DragPreviewShowsContentPixmap,  ///< the default configuration for
                                            ///< non opaque operations

        NonOpaqueWithWindowFrame =
            DefaultNonOpaqueConfig |
            DragPreviewHasWindowFrame  ///< the default configuration for non
                                       ///< opaque operations that show a real
                                       ///< window with frame
    };
     ...
     * The XmlMode enables / disables the auto formatting for the
     * XmlStreamWriter. If auto formatting is enabled, the output is intended
     * and line wrapped. The XmlMode XmlAutoFormattingDisabled is better if you
     * would like to have a more compact XML output - i.e. for storage in ini
     * files.
     */
