    ...
    QPixmap createHighDpiDropIndicatorPixmap(const QSizeF& size,
        ...
        switch (DockWidgetArea) {
            case TopDockWidgetArea:
                ...
                areaLine =
                    QLineF(areaRect.bottomLeft(), areaRect.bottomRight());
                break;
            ...
            case LeftDockWidgetArea:
                areaRect = QRectF(baseRect.x(), baseRect.y(),
                                  baseRect.width() * .5f, baseRect.height());
...
//============================================================================
CDockOverlay::~CDockOverlay() { delete d; }

//============================================================================
void CDockOverlay::setAllowedAreas(DockWidgetAreas areas) {
    if (areas == d->AllowedAreas) return;
    d->AllowedAreas = areas;
    d->Cross->reset();
}

//============================================================================
DockWidgetAreas CDockOverlay::allowedAreas() const { return d->AllowedAreas; }
...
DockWidgetArea CDockOverlay::dropAreaUnderCursor() const {
    ...
    CDockAreaWidget* DockArea =
        qobject_cast<CDockAreaWidget*>(d->TargetWidget.data());
    if (!DockArea) {
        return Result;
    }
    ...
    if (DockArea->allowedAreas().testFlag(CenterDockWidgetArea) &&
        ...
        DockArea->titleBarGeometry().contains(
            DockArea->mapFromGlobal(QCursor::pos()))) {
...
//============================================================================
bool CDockOverlay::dropPreviewEnabled() const { return d->DropPreviewEnabled; }
...
//============================================================================
QRect CDockOverlay::dropOverlayRect() const { return d->DropAreaRect; }
...
//============================================================================
CDockOverlayCross::~CDockOverlayCross() { delete d; }
...
void CDockOverlayCross::setupOverlayCross(CDockOverlay::eMode Mode) {
    ...
    areaWidgets.insert(BottomDockWidgetArea, d->createDropIndicatorWidget(
                                                 BottomDockWidgetArea, Mode));
    areaWidgets.insert(LeftDockWidgetArea,
                       d->createDropIndicatorWidget(LeftDockWidgetArea, Mode));
    areaWidgets.insert(CenterDockWidgetArea, d->createDropIndicatorWidget(
                                                 CenterDockWidgetArea, Mode));
#if QT_VERSION >= 0x050600
...
//============================================================================
void CDockOverlayCross::setIconColor(eIconColor ColorIndex,
                                     const QColor& Color) {
...
DockWidgetArea CDockOverlayCross::cursorLocation() const {
    ...
    while (i.hasNext()) {
        i.next();
        if (d->DockOverlay->allowedAreas().testFlag(i.key()) && i.value() &&
            i.value()->isVisible() && i.value()->geometry().contains(pos)) {
            return i.key();
        }
    }
    return InvalidDockWidgetArea;
...
#else
        auto ComponentColor =
            ColorListEntry.split('=', QString::SkipEmptyParts);
#endif
...
//============================================================================
QString CDockOverlayCross::iconColors() const { return QString(); }
