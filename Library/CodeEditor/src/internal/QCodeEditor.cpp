...
void QCodeEditor::keyPressEvent(QKeyEvent* e) {
    ...
    if (!completerSkip) {
...
#if QT_VERSION >= 0x050A00
        int tabCounts = indentationLevel * fontMetrics().averageCharWidth() /
                        tabStopDistance();
#else
        int tabCounts = indentationLevel * fontMetrics().averageCharWidth() /
                        tabStopWidth();
        ...
        // Have Qt Edior like behaviour, if {|} and enter is pressed indent the
        // two parenthesis
...
#else
            indentationLevel +=
                tabStopWidth() / fontMetrics().averageCharWidth();
#endif
        }
    }
