...
Connection::Connection(Node& nodeIn, PortIndex portIndexIn, Node& nodeOut,
                       PortIndex portIndexOut, TypeConverter typeConverter)
    : _uid(QUuid::createUuid()),
      _outNode(&nodeOut),
      _inNode(&nodeIn),
...
void Connection::setGraphicsObject(
    ...
    if (requiredPort() != PortType::None) {
        ...
        QPointF pos = node->nodeGeometry().portScenePosition(
...
void Connection::setNodeToPort(Node& node, PortType portType,
                               PortIndex portIndex) {
...
void Connection::removeFromNodes() const {
    ...
    if (_outNode)
        _outNode->nodeState().eraseConnection(PortType::Out, _outPortIndex,
                                              id());
}
...
ConnectionState& Connection::connectionState() { return _connectionState; }
...
NodeDataType Connection::dataType(PortType portType) const {
    if (_inNode && _outNode) {
        ...
        PortIndex index =
            (portType == PortType::In) ? _inPortIndex : _outPortIndex;
