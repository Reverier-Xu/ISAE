...
class NodeConnectionInteraction {
   public:
    ...
    /// a converter node to the scene, and connect it properly 2)   Assign node
    /// to required port in Connection 3)   Assign Connection to empty port in
    /// NodeState 4)   Adjust Connection geometry 5)   Poke model to initiate
    /// data transfer
