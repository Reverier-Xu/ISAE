...
fitness = ngram_score('Modules/DataFlow/cracks/fmorse_quadgrams.txt') # load our quadgram model
...
def break_fracmorse(ctext):
    ...
    out = str(maxscore)+'FractionatedMorse key:'+''.join(maxkey)+ 'decrypt: '+FracMorse(maxkey).decipher(ctext)+'\n'
    for i in range(1000):
